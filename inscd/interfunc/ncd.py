...
class NCD_IF(_InteractionFunction, nn.Module):
    def __init__(self, knowledge_num: int, hidden_dims: list, dropout, device, dtype,step):
        super().__init__()
        ...
        layers.update(
            {
                ...
                'linear{}'.format(len(self.hidden_dims)): nn.Linear(
                    self.hidden_dims[len(self.hidden_dims) - 1], 5, dtype=self.dtype
                ),
                #'activation{}'.format(len(self.hidden_dims)): nn.Softmax()
            }
        )
