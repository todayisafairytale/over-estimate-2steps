...
class DataHub:
    ...
    def detach_labels(self, set_type) -> list:
        if set_type not in self.__set_type_map.keys():
            raise ValueError("Dataset \"{}\" does not exist. If you create your new dataset via \"load_data()\", "
                             "the parameter \"dataset\" is one of the {}".format(set_type, self.__set_type_map.keys()))
        L=self.__set_type_map[set_type][:, -1].T.tolist()
        ...
        if label is True:
            if tmp_set.shape[1] == 3:
                tensor_dataset = tud.TensorDataset(
                    ...
                    torch.tensor(self.S[np.array(tmp_set[:,0],dtype=int),:],dtype=dtype),
                    torch.tensor(self.theta_tuda[np.array(tmp_set[:,0],dtype=int),:],dtype=dtype)
                )
            else:
    ...
    def get_causal_graph(self):
        k2k={}
        for k1_id in range(self.g_mask[0]):
            k2k[k1_id]=[]
        for k1_id,k2_id in zip(*np.where(self.g_mask != 0)):
            k2k[k1_id].append(k2_id)
        return k2k
